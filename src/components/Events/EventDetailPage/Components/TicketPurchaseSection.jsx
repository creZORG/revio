...
import commonStyles from '../../Common/Button.module.css'; // For common button styles

const TicketPurchaseSection = ({ event, onProceedToCheckout, setShowMpesaModal, setMpesaAmount, setMpesaPhoneNumber }) => {
    const { showNotification } = useNotification();
    const { cartItems, updateCartItemQuantity } = useCart(); // Get cart state and actions
    ...
    const handleQuantityChange = useCallback((ticketId, change) => {
        ...
        const newQty = currentQty + change; // No Math.max(0, ...) here, let updateCartItemQuantity handle it
        ...
        updateCartItemQuantity(ticket.id, newQty); // Call the CartContext function
    ...
    return (
        <section className={`${styles.sectionContent} ${styles.ticketsSection}`}>
                            ...
                            <div key={ticket.id} className={`${styles.ticketItem} ${isTicketDisabled ? styles.disabled : ''}`}>
                                ...
                                <div className={styles.ticketQuantityControl}>
                                    <button
                                        ...
                                        disabled={isTicketDisabled || currentQty <= 0} // Disable if qty is 0
                                    >
                                        <FaMinus />
                                    </button>
