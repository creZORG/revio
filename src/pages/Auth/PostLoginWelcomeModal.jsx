...
import { useAuth } from '../../hooks/useAuth.js'; // Import useAuth to get current user/profile
...
const PostLoginWelcomeModal = ({ isOpen, onClose }) => {
  ...
  const [organizerRequestStatus, setOrganizerRequestStatus] = useState(null); // null, 'pending', 'approved', 'rejected'
  ...
  useEffect(() => {
    ...
    // Fetch organizer request status if user is 'user' role
  ...
  if (!currentUser || !isAuthenticated || userRole === 'admin') { // Don't show for admin
    return null;
  }

  const isEmailVerified = currentUser.emailVerified;
  const isInfluencerReady = daysLoggedIn >= 7; // Influencer ready after 7 days logged in
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Welcome, ${currentUser.displayName || currentUser.email.split('@')[0]}!`}>
      <div className={styles.modalContent} style={{textAlign: 'center', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px'}}>
        ...
        {userRole === 'user' && ( // Only show these options if current role is 'user'
